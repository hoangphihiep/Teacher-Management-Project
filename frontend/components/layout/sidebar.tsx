...
export function Sidebar({ className }: SidebarProps) {
  ...
  console.log("Sidebar component rendered, user:", user)
          ...
          <Link href="/profile">
            <div className="flex items-center gap-3 p-3 rounded-lg bg-royal-50 border border-royal-100 hover:bg-royal-100 transition-colors cursor-pointer">
              <Avatar className="h-10 w-10">
                <AvatarImage src="/placeholder-avatar.jpg" alt="Giáo viên" />
                <AvatarFallback className="bg-royal-500 text-white">
                  {typeof window !== "undefined" && user?.fullName ? getInitials(user.fullName) : "GV"}
                </AvatarFallback>
              </Avatar>
